    ...
    <aside class="h-screen bg-[#acb0b3] transition-all duration-300 overflow-hidden pl-2 ml-2 float-right shadow-inner shadow-[#29627e]/30 italic text-[#29627e]" :class="rightIsCollapsed ? 'w-12' : 'w-50'" @mouseenter="rightIsCollapsed = false" @mouseleave="rightIsCollapsed = true">
      <div v-if="selectedNode" class="p-4 h-full flex flex-col">
        ...
        <div class="bg-[#29627e]/10 p-3 rounded-md mb-5 border border-[#29627e]/20">
          <h3 class="mb-2 text-[1.1em] text-[#29627e] font-semibold">
            {{ selectedNode.label }}
          </h3>
          <p class="text-[0.9em] text-[#666]">ID: {{ selectedNode.id }}</p>
        </div>
        <div class="flex flex-col gap-2.5">
          <button class="action-btn py-2.5 px-4 border border-[#acb0b3] bg-white rounded-md cursor-pointer text-sm text-[#29627e] font-medium transition-all hover:bg-f0f8ff hover:border-[#29627e] hover:-translate-y-0.5 hover:shadow-md text-left" @click="editNode">编辑</button>
          <button class="action-btn py-2.5 px-4 border border-[#acb0b3] bg-white rounded-md cursor-pointer text-sm text-[#29627e] font-medium transition-all hover:bg-f0f8ff hover:border-[#29627e] hover:-translate-y-0.5 hover:shadow-md text-left" @click="addChildNode">添加子节点</button>
          <button class="action-btn danger py-2.5 px-4 border border-[#fecaca] bg-white rounded-md cursor-pointer text-sm text-[#dc2626] font-medium transition-all hover:bg-fef2f2 hover:border-[#fca5a5] hover:-translate-y-0.5 hover:shadow-md text-left" @click="deleteNode">删除</button>
        </div>
      </div>
      <div v-else class="no-selection p-4 text-center text-[#666]">
